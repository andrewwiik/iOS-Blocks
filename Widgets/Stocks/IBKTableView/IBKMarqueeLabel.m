...
- (id)initWithFrame:(CGRect)frame {
    return [self initWithFrame:frame duration:7.0 andFadeLength:0.0 alsoIBKSizing:kIBKLabelSizingMedium];
}

- (id)initWithFrame:(CGRect)frame duration:(NSTimeInterval)aLengthOfScroll andFadeLength:(CGFloat)aFadeLength alsoIBKSizing:(int)sizing {
    self = [super initWithFrame:frame];
    if (self) {
        [self setupLabelWithIBKSizing:sizing];
...
- (id)initWithFrame:(CGRect)frame rate:(CGFloat)pixelsPerSec andFadeLength:(CGFloat)aFadeLength alsoIBKSizing:(int)sizing {
    self = [super initWithFrame:frame];
    if (self) {
        [self setupLabelWithIBKSizing:sizing];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self) {
        [self setupLabelWithIBKSizing:kIBKLabelSizingMedium];
...
- (void)setupLabelWithIBKSizing:(int)sizing {
    // Basic UILabel options override
    self.clipsToBounds = YES;
    self.numberOfLines = 1;
    self.ibkSizing = sizing;
    self.marqueeType = MLContinuous;
    self.trailingBuffer = 30; // By default.
    
    // Create first sublabel
    self.subLabel = [[IBKLabel alloc] initWithFrame:self.bounds];
    self.subLabel.tag = 700;
    self.subLabel.layer.anchorPoint = CGPointMake(0.0f, 0.0f);
    
    [self.subLabel setLabelSize:sizing];
...
- (void)updateSublabelAndLocationsAndBeginScroll:(BOOL)beginScroll {
    ...
    switch (self.marqueeType) {
        ...
        case MLContinuousReverse:
        {
            ...
            NSArray *labels = [self allSubLabels];
            if (labels.count < 2) {
                IBKLabel *secondSubLabel = [[IBKLabel alloc] initWithFrame:CGRectOffset(self.homeLabelFrame, -awayLabelOffset, 0.0f)];
                secondSubLabel.numberOfLines = 1;
                secondSubLabel.tag = 701;
                secondSubLabel.layer.anchorPoint = CGPointMake(0.0f, 0.0f);
                
                [secondSubLabel setLabelSize:self.ibkSizing];
            ...
            // Set sublabel frames
            for (IBKLabel *sl in [self allSubLabels]) {
